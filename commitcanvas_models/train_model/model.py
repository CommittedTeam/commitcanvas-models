...
class model_helpers:
  ...
  def train(self):
    ...
    pipeline = self.build_pipline()
    pipeline = pipeline.fit(self.train_data,self.train_labels)
    
    return pipeline

  def report(self,pipeline,predicted):

    # display classification report
    types = self.train_labels.unique()
    
    print(
    classification_report(self.train_labels,predicted, target_names=types)
    )

    plot_confusion_matrix(pipeline, self.train_data, self.train_labels, display_labels=types,cmap='Blues',normalize="true")
    plt.show()    

  def save(self,pipeline,save):
